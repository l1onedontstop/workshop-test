...
function getSelfInsight(projectPath: string): string {
  ...
  try {
    ...
    const parts: string[] = []
    if (a.accountSummary) parts.push(`当前账号定位：${a.accountSummary}`)
    if (a.contentPillars?.length) parts.push(`内容支柱：${a.contentPillars.join('、')}`)
    if (a.styleFeatures?.length) parts.push(`风格特征：${a.styleFeatures.join('、')}`)
    if (a.strengths?.length) parts.push(`强项：${a.strengths.join('；')}`)
    if (a.weaknesses?.length) parts.push(`弱项：${a.weaknesses.join('；')}`)
    if (a.personaAdvice?.reasoning) parts.push(`人设建议：${a.personaAdvice.reasoning}`)
    if (a.personaAdvice?.recommended) {
      const recMap: Record<string, string> = { keep: '建议继续现有方向', optimize: '建议在现有基础上优化调整', transform: '建议转型' }
      parts.push(`推荐路径：${recMap[a.personaAdvice.recommended] || ''}`)
...
export function buildBlueprintPrompt(answers: Record<string, string>, projectPath?: string) {
  const parts = [
    '## 用户画像',
  ...
  // Inject self-account analysis if available
  if (projectPath) {
    const selfInsight = getSelfInsight(projectPath)
    if (selfInsight) {
      parts.push('', '## 用户现有账号分析（重要参考）', selfInsight)
    }
  }

  parts.push('', '请生成完整的 IP 打造蓝图。')
  return parts.filter(Boolean).join('\n')
}

export async function generateBlueprint(answers: Record<string, string>, projectPath?: string): Promise<any> {
  info('ip-strategy', 'Generating blueprint')
  const messages = [
    { role: 'system' as const, content: IP_BLUEPRINT_PROMPT },
    { role: 'user' as const, content: buildBlueprintPrompt(answers, projectPath) }
  ]
  try {
...
export function registerIPStrategyHandlers(): void {
  ipcMain.handle('ip-strategy:generate', async (_e, answers: Record<string, string>, projectPath?: string) => {
    return generateBlueprint(answers, projectPath)
  })
