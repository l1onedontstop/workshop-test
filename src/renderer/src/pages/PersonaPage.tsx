        ...
        <div className="max-w-3xl mx-auto">
          ...
          {!persona && !loading && (
            <div className="text-center py-20">
              <Users size={48} className="text-ink-disabled mx-auto mb-4" />
              <h3 className="text-lg font-medium text-ink-tertiary mb-2">还没有受众画像</h3>
          ...
          {loading && (
            <div className="flex items-center justify-center py-20">
              ...
              <span className="ml-3 text-ink-tertiary">AI 正在分析观众数据...</span>
            </div>
          )}
          ...
          {persona && (
            <div className="space-y-6">
              ...
              <div className="p-5 rounded-xl bg-brand-50 border border-brand-200">
                ...
                <p className="text-ink-secondary text-sm">{persona.summary}</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <AudienceCard
                  title="核心受众"
                  icon={<Users size={16} className="text-info-text" />}
              ...
              {persona.contentAdvice?.toneAdvice && (
                <div className="p-4 rounded-xl bg-black/[0.02] border border-rule-subtle">
                  ...
                  <p className="text-sm text-ink-secondary">{persona.contentAdvice.toneAdvice}</p>
                </div>
              )}
  ...
  return (
    <div className="p-4 rounded-xl bg-black/[0.02] border border-rule-subtle">
      ...
      <div className="space-y-3">
        {data.age && (
          <div>
            <p className="text-[10px] text-ink-disabled">年龄段</p>
            <p className="text-sm text-ink-secondary">{data.age}</p>
          </div>
        )}
        {data.occupation && (
          <div>
            <p className="text-[10px] text-ink-disabled">典型职业</p>
            <p className="text-sm text-ink-secondary">{data.occupation}</p>
          </div>
        )}
        {data.interests?.length > 0 && (
          <div>
            <p className="text-[10px] text-ink-disabled mb-1">兴趣</p>
            <div className="flex flex-wrap gap-1">
              {data.interests.map((s, i) => (
                <span key={i} className="px-2 py-0.5 rounded-full text-[10px] bg-black/[0.04] border border-rule text-ink-tertiary">
                  {s}
                </span>
              ))}
            </div>
        ...
        {data.painPoints?.length > 0 && (
          <div>
            <p className="text-[10px] text-ink-disabled mb-1">痛点</p>
            <div className="flex flex-wrap gap-1">
        ...
        {data.whyFollow && (
          <div>
            <p className="text-[10px] text-ink-disabled">关注原因</p>
            <p className="text-xs text-ink-tertiary">{data.whyFollow}</p>
          </div>
        )}
      </div>
    </div>
